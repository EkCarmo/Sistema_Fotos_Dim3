...
st.write("Suba a foto do material. O sistema isola, limpa fundos complexos e gerencia a memória automaticamente.")
...
def carregar_modelo_ia(modo_roupa=False):
    try:
        if modo_roupa:
            # IA especialista em moda: isola tecidos e ignora pescoço/corpo de manequins e modelos
...
# 4. Função de Recorte com TRAVA DE MEMÓRIA e SELEÇÃO DE IA
@st.cache_data(max_entries=2, show_spinner=False)
def recortar_fundo(imagem_bytes, usar_alta_precisao, forca_desconexao, modo_transp, filtro_tam, modo_roupa):
...
modo_vidro = st.sidebar.checkbox(
    ...
    value=False, 
    help="Marque ESTA CAIXA se o produto for de vidro, acrílico ou tiver partes transparentes."
)
...
filtro_ilhas = st.sidebar.slider(
    ...
    value=15, 
    step=2, 
    help="Apaga objetos desconectados menores que essa porcentagem. Deixe em 0% APENAS para kits com peças soltas na mesa."
)
...
forca_sep = st.sidebar.slider(
    ...
    value=0, 
    step=2, 
    help="Deixe em 0 se a foto não tiver nada encostado. Se houver clipes ou papéis colados no produto, aumente gradualmente."
)
...
if arquivo_enviado is not None:
    ...
    with st.spinner("🤖 IA processando, isolando e centralizando o produto..."):
        img_sem_fundo = recortar_fundo(bytes_arquivo, modo_precisao, forca_sep, modo_vidro, filtro_ilhas, modo_roupa)
